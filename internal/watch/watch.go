...
// Subscribe registers a client and loads its cursor.
...
func (m *Manager) Deliver(clientID string, version uint64) error {
	// The cursor moves before the acknowledgement is durable, so a failed ack
	// after the move skips the push forever.
	if err := m.AdvanceCursor(clientID, version); err != nil {
		return err
	}
	return m.Ack(clientID, version)
}
